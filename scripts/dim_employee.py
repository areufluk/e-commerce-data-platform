...
def transform_dim_employee():
    bucket_name = "e-commerce-data-platform"
    source_file_path = f"gs://{bucket_name}/raw/employees.csv"
    destination_file_path = f"gs://{bucket_name}/transform/employee"

    spark = create_spark_session(
        app_name="Transform dim employee"
    )
    ...
    # Change column name
    df = df \
        .withColumnRenamed("EmployeeID", "id") \
